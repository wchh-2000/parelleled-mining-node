...
void *Mining_thread(void *arg)
{
	...
	Block_init(a->block,a->hash);
	find=Mining(a->block,a->hash,a->str,n0);
	//printf("\n%d\n",Verify(a->str,a->hash,n0));
	return; 
}
...
int Mining(BLOCK *p,BYTE *hash,BYTE *str,int n0)
...
//试不同nonce，区块转为字符数组 验证verify 存下最终nonce, hash 
{
	...
	if(v)//满足要求
	{
		p->nonce1=i-1;
		p->nonce2=j-1;
		printf("\nfind nonce1:%lld nonce2:%lld\nhash:",i-1,j-1);
		for(j=0;j<4;j++)
		printf("%x ",hash[j]);
		/*
		puts("block str:");
		int k;
		for (k = 0; k < blockBytes; k++)
			printf("%x ", str[k]);
		printf("\n");//验证字符串数组*/ 
		return 1;
	}
...
void print_block(BYTE *str)
{
	...
	for(j=0;j<4;j++)
		printf("%x ",str[j]);
	printf("\nnonce1:%lld nonce2:%lld\n",str2long(&str[40]),str2long(&str[48]));//注意lld才能正确显示long long 
	/*
	int n,N=5;
	for(n=0;n<N;n++)
	{
